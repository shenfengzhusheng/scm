...
import org.xfs.scm.platform.config.netty.socket.util.SocketUtil;
...
public class CustomSocketClient {
	...
	public static void customSend(int thread)throws Exception{
		...
		new Thread(new Runnable() {
			...
			public void run() {
				while(true) {
					try {
						CustomSocketClient.readStream(input);
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
	...
	public static void readStream(InputStream inputStream,String requestId)throws Exception {
		if(inputStream!=null) {
			...
			System.out.println("get message from server is:"+vo);
	...
	public static void readStream(InputStream inputStream)throws Exception {
		if(inputStream!=null) {
			...
			vo.setLength(length);
			vo.setProtocol(protocol);
			vo.setRequestId(UUID.randomUUID().toString().replaceAll("-", ""));
			vo.setSequence(sequence);
			vo.setType((byte)type);
			vo.setVersion((byte)version);
