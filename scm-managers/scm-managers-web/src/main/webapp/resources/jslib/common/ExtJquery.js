var xfs = xfs || {};
xfs.data = xfs.data || {};// 用于存放临时的数据或者对象
 ...
 * 增加命名空间功能
 * 
 * 使用方法：xfs.ns('jQuery.bbb.ccc','jQuery.eee.fff');
 */
xfs.ns = function() {
	var o = {}, d;
 ...
 * 将form表单元素的值序列化成对象
 * 
 * @example xfs.serializeObject($('#formId'))
 * 
 * @author 刘治
 * 
 * @requires jQuery
 ...
 * @returns object
 */
xfs.serializeObject = function(form) {
	var o = {};
 ...
 * @author 刘治
 * 
 * @example xfs.formatString('字符串{0}字符串{1}字符串','第一个变量','第二个变量');
 * 
 * @returns 格式化后的字符串
 */
xfs.formatString = function(str) {
 ...
 * @returns list
 */
xfs.stringToList = function(value) {
 ...
 * 
 * @example xfs.cookie('the_cookie', 'the_value');
 * @desc Set the value of a cookie.
 * @example xfs.cookie('the_cookie', 'the_value', { expires: 7, path: '/', domain: 'jquery.com', secure: true });
 * @desc Create a cookie with all available options.
 * @example xfs.cookie('the_cookie', 'the_value');
 * @desc Create a session cookie.
 * @example xfs.cookie('the_cookie', null);
 ...
 * @author Klaus Hartl/klaus.hartl@stilbuero.de
 */
xfs.cookie = function(key, value, options) {
