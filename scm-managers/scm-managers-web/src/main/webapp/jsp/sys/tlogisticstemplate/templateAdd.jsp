...
<script type="text/javascript">
	...
	/*var submitForm=function($dialog,$grid,$pjq) {
		if($('form').form('validate')){
		    ...
		    if($(':input[name="id"]').val().length>0){
		    	url=xfs.contextPath +'/tlogisticstemplate/update.do';
			}else{
		   		url=xfs.contextPath +'/tlogisticstemplate/add.do';
		   	}
	...
	/*$(function() {
		if ($(':input[name="id"]').val().length > 0) {
			...
			$.post(xfs.contextPath + '/tlogisticstemplate/getById.do', {
			...
			//alert("0");
				if (result.id != undefined) {
					$('form').form('load', {
						...
						'logisticsCompanyId':result.logisticsCompanyId,
					});
				}
	...
	function SubmitForm() {
	    ...
	    $.post(xfs.contextPath +'/logisticstemplate/add.do', dataParam, function (result) {
	        if (result.success) {
	            $.messager.progress('close');
	            alert(result.msg);
	            window.location.href = xfs.contextPath + '/jsp/sys/tlogisticstemplate/tlogisticstemplate.jsp';
	        }
	        else {
	            $.messager.progress('close');
	            alert(data.message);
...
<!-- <form method="post" class="form"> -->
        ...
        <div region="north" title="基本信息" style="height:150px">
            <table cellpadding="0" class="formTable">
                <tr>
                    <th ><font color='red'>33*</font>物流公司</th>
                    <td >
                        <input id="carryId" name="carryId" class="w110" />                                         
                    </td>
                    <th ><i class="require">*</i>模板名称</th>
                    <td><input type="text" class="text w110" name="templateName" id="templateName" /></td>
                    <th><i class="require">*</i>模板类型</th>
                    <td>
                        <select id="templateType" name="templateType" class="w110">
