...
<script type="text/javascript">
    ...
    function SubmitForm() {
        ...
        if (IsNull(dataParam.templateName)) {
            alert('请输入模板名称');
            return;
        }
        ...
        if (IsNull(dataParam.templateContent)) {
            alert('模板不能为空');
            return;
        }

        $.post(xfs.contextPath +'/tlogisticstemplate/update.do', dataParam, function (data) {
            if (data.status == 1) {
                $.messager.progress('close');
                alert(data.message);
                window.location.href = xfs.contextPath + '/jsp/sys/tlogisticsTemplate.jsp';
            }
            else {
                $.messager.progress('close');
                alert(data.message);
