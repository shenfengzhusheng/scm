...
<c:if test="${fn:contains(sessionInfo.resourceList, '/tlogisticstemplate/getById')}">
	<script type="text/javascript">
		$.canGet = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/tlogisticstemplate/update')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/tlogisticstemplate/remove')}">
	<script type="text/javascript">
		$.canRemove = true;
	</script>
</c:if>
<script type="text/javascript">
	...
	var addFun = function() {
		alert('ddddddd');
		 window.location.href = xfs.contextPath + '/jsp/sys/tlogisticstemplate/templateAdd.jsp';
	};
	...
	var editFun = function() {
        ...
        window.location.href = xfs.contextPath + '/jsp/sys/tlogisticstemplate/tlogisticstemplateEdit.jsp'+ "?id=" + rows[0]["id"];
	};
	...
	/*var unEnableFun =function(){
		parent.$.messager.confirm('询问', '您确定要停用此模板？', function(r) {
			if (r) {
				$.post(xfs.contextPath + '/tlogisticstemplate/remove.do', {
					id : id
				}, function() {
	...
	$(function() {
		grid = $('#grid').datagrid({
			title : '',
			url : '${pageContext.request.contextPath}/tlogisticstemplate/grid.do',
			striped : true,
			rownumbers : true,
			pagination : true,
			singleSelect : true,
              ...
              <td>
                ...
                <table>
							<tr>
								...
								<td>物流公司</td>
								<td><input name="logisticsCompanyId" style="width: 80px;" /></td>
