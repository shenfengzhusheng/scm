...
<c:if test="${fn:contains(sessionInfo.resourceList, '/info/getById')}">
	<script type="text/javascript">
		$.canGet = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/info/update')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/info/remove')}">
	<script type="text/javascript">
		$.canRemove = true;
	</script>
</c:if>
<script type="text/javascript">
	...
	var addFun = function() {
		var dialog = parent.xfs.modalDialog({
			title : '添加任务信息',
			url : xfs.contextPath + '/jsp/sys/job/info/infoForm.jsp',
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	var showFun = function(id) {
		var dialog = parent.xfs.modalDialog({
			title : '查看任务信息',
			url : xfs.contextPath + '/jsp/sys/job/info/infoForm.jsp?id=' + id
		});
	};
	...
	var removeFun = function(id) {
		...
		parent.$.messager.confirm('询问', '您确定要删除此记录？', function(r) {
			if (r) {
				$.post(xfs.contextPath + '/info/remove.do', {
					jobId : id
				}, function() {
	...
	$(function() {
		grid = $('#grid').datagrid({
			title : '',
			url : '${pageContext.request.contextPath}/info/grid.do',
		...
		//		sortable : true
				...
				formatter : function(value, row) {
					var str = '';
					   if($.canGet){
							str += xfs.formatString('<img class="iconImg ext-icon-note" title="查看" onclick="showFun(\'{0}\');"/>', row.jobId);

					   }
 						if($.canEdit){
 							str += xfs.formatString('<img class="iconImg ext-icon-note_edit" title="编辑" onclick="editFun(\'{0}\');"/>', row.jobId);

					   }
 						if($.canRemove){
 							str += xfs.formatString('<img class="iconImg ext-icon-note_delete" title="删除" onclick="removeFun(\'{0}\');"/>', row.jobId);
 						}
					return str;
				}
			} ,{
				...
				<td>
					<form id="searchForm">
						<table>
							<tr>
								<td>任务JAVA类型</td>
								<td><input name="jobclass" class="easyui-textbox" style="width: 150px;" /></td>
								<!-- <td>是否启用</td>
