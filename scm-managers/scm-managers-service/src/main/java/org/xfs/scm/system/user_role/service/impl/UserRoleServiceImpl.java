package org.xfs.scm.system.user_role.service.impl;
...
import org.xfs.scm.system.user_role.dao.UserRoleMapper;
...
public class UserRoleServiceImpl extends NBaseServiceImpl<UserRole> implements UserRoleServiceI {
	...
	@Transactional(readOnly=false,propagation=Propagation.REQUIRED)
	@Override
	public boolean grantRole(Long id, String ids) {
		...
		for(UserRole userRole:list){
			...
			for(int i=0,size=arr.length;i<size;i++){
				if(userRole.getRid().toString().equals(arr[i])){
					deleteFlag=false;
					break;
				}
			}
