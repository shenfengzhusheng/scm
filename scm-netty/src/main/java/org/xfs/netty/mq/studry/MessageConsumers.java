    ...
    public static void main(String[]args){
        ...
        try {
            ...
            Session session=connection.createSession(
                    false,//是否启用事【消费时不能设置事务否则需session.commit()】
            ...
            /**
             * 第六步:最后我们使用JMS规范的TextMessage形式创建数据(通过Session对象),并且MessageProducer的send方未能去发送数数据,同理客户端使用MessageConsume消费消息
             */
            while(true){
                 //   TimeUnit.SECONDS.sleep(1);
                    TextMessage msg=(TextMessage)messageConsumer.receive();
                    msg.acknowledge();//手工签收消息，别起一个线程（TCP）通知MQ签收，确认签 收
